...
class TestProjeto(unittest.TestCase):
    ...
    def test_adiciona_usuario(self):
        ...
        r = requests.put("http://localhost:8000/usuarios?nome=Victor&email=%40&cidade=SP")
        self.assertEqual(r.status_code,200)

        # Checa se o usuario existe.
        r = requests.get("http://localhost:8000/usuarios?nome=Victor")
        id = r.json()
        self.assertIsNotNone(id)

        # Tenta achar um usuario inexistente.
        r = requests.get("http://localhost:8000/usuarios?nome=Jonas")
        id = r.json()
    ...
    def test_remove_usuario(self):
        ...
        email = '@'
        cidade = 'SP'
        r = requests.put("http://localhost:8000/usuarios?nome=Gustavo&email=%40&cidade=SP")
        r = requests.get("http://localhost:8000/usuarios?nome=Victor")
        id = r.json()
